          ...
          <div className="flex-1">
            <div className={containerStyle}>
              ...
              <div className="border-t border-gray-200">
                {faqList.map((faq) => (
                  <div key={faq.csNo} className="border-b border-gray-100 last:border-none">
                    <div
                      className={`flex items-center gap-3 md:gap-6 py-5 md:py-8 px-2 md:px-5 cursor-pointer transition-all hover:bg-gray-50/50 ${
                        openId === faq.csNo ? "bg-gray-50/80" : ""
                      }`}
                      onClick={() => toggleAccordion(faq.csNo)}>
                      
                      {/* Q. 아이콘 수직 중앙 정렬 및 공통 폰트 적용 */}
                      ...
                      <div className="flex-1">
                        <p className={`fs-up-2 leading-snug ${
                          openId === faq.csNo ? "font-bold text-blue-600" : "text-gray-800 font-medium"
                        }`}>
                          {faq.csTitle}
                        </p>
                      </div>

                      <span className="hidden lg:block fs-down-2 text-gray-400 w-28 text-right shrink-0 whitespace-nowrap">
                        {faq.csRegDate?.substring(0, 10)}
                      </span>

                      <span className={`text-gray-300 transition-transform pt-1.5 ${
                        openId === faq.csNo ? "rotate-180 text-blue-400" : ""
                      }`} style={{ fontSize: '10px' }}>
                        ▼
                      </span>
                    </div>
                    ...
                    {/* 답변 영역: 공통 스타일 이식 */}
                    {openId === faq.csNo && (
                      <div className="bg-gray-50/50 px-5 md:px-16 py-8 md:py-10 border-t border-gray-100 flex flex-col">
                        <div className="flex gap-3 md:gap-6">
                          <span className="fs-up-4 text-blue-500 font-bold shrink-0">A.</span>
                        ...
                        <div className="flex items-center justify-end gap-3 pt-10 fs-down-1 text-gray-400 self-end">
                          <div className="flex items-center gap-1.5">
                            ...
                            <span>{faq.csRegDate?.substring(0, 10)}</span>
                          </div>
                          <div className="w-[1px] h-2.5 bg-gray-300"></div>
                          <div className="flex items-center gap-1.5">
                            {/* 등록시간 문구는 주석처리 요청하신 대로 반영했습니다 */}
                            <span>{faq.csRegDate?.substring(11, 16)}</span>
                          </div>
                        </div>
                      </div>
                    )}
