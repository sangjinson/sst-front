...
const CommunityLifeWrite = () => {
  ...
  const currentPost = isEditMode
    ? getAllLifePosts().find((p) => p.id === Number(id))
    : null;
          ...
          <form onSubmit={handleSubmit} className="p-8 space-y-8">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              <div className="relative">
                ...
                <button
                  ...
                  className="w-full h-12 px-4 border border-gray-200 rounded-lg bg-white flex justify-between items-center"
                >
                  <span
                    className={
                  ...
                  <div className="absolute z-50 w-full bg-white border border-gray-200 rounded-lg shadow-xl max-h-60 overflow-y-auto">
                    {gyeonggiRegions.map((region) => (
                      <button
                        ...
                        className="w-full text-left px-4 py-3 hover:bg-green-50 hover:text-[#0F9B73] cursor-pointer text-sm text-gray-700 transition"
                      >
                        {region}
                      </button>
                    ))}
            ...
            <div>
              ...
              <textarea
                ...
                className="w-full px-4 py-4 border border-gray-200 rounded-lg outline-none resize-none text-gray-700 leading-relaxed text-base focus:border-[#0F9B73] transition"
              />
            </div>
                      ...
                      <div className="flex-1 bg-gray-50 rounded-2xl p-4 flex gap-3">
                        ...
                        <div className="flex-1 min-w-0">
                          ...
                          <input
                            value={c.desc || ""}
                            onChange={(e) =>
                              updateCourseItem(idx, "desc", e.target.value)
                            }
            ...
            <div>
              ...
              <div className="flex flex-wrap gap-2 border border-gray-200 rounded-lg px-3 py-3 focus-within:border-[#0F9B73] transition">
                {tags.map((tag) => (
                  <button
                    key={tag}
                    type="button"
                    onClick={() => setTags(tags.filter((t) => t !== tag))}
                    className="px-3 py-1 rounded-full bg-emerald-50 text-emerald-600 text-xs font-bold"
                  >
                    #{tag} ✕
                  </button>
                ))}
            ...
            <div className="flex justify-end gap-3 pt-6 border-t border-gray-100">
              <button
                type="button"
                onClick={() => navigate(-1)}
                className="px-5 py-2.5 bg-gray-100 text-gray-600 font-semibold rounded-lg hover:bg-gray-200 transition"
              >
                취소
              </button>
