...
import { hotplacePosts } from "./communityHotplaceData";
...
const CommunityHotplace = () => {
  ...
  // 카드 이미지 비율 설정
  // 인스타/핀터레스트 느낌을 주기 위해 카드마다 다른 비율 사용
          ...
          {filteredPosts.map((post) => {
            ...
            return (
              <article
                ...
                {/* 카드 상단: 제목 + 이미지 클릭 시 상세 페이지 이동 */}
                <button
                  type="button"
                  onClick={() => navigate(`/showcase/hotplace/view/${post.id}`)}
                  className="block w-full text-left">
                  <div
                    className={`relative w-full overflow-hidden bg-gray-100 ${getImageRatio(
                      post.size
                    )}`}>
                    <img
                      src={post.img}
                ...
                {/* 카드 하단: 내용, 해시태그, 작성자, 장소, 조회수, 댓글수, 찜수 */}
                <div className="p-4 bg-white">
                  ...
                  <div className="mb-4 flex flex-wrap gap-2">
                    {post.hashtags.map((tag) => (
                      <span
                        key={tag}
                        className="fs-down-1 font-semibold text-emerald-600 hover:text-emerald-700">
                        #{tag}
                      </span>
                    ))}
                  </div>
