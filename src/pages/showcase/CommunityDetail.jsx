  ...
  return (
    <div className="max-w-7xl mx-auto px-4 py-6 md:py-10">
      ...
      <div className="mb-8 mt-4 md:mt-12">
        <h2 className="text-xl md:text-2xl font-bold mb-2 text-gray-800">뽐낼거리</h2>
      ...
      <div className="flex flex-col lg:flex-row gap-8 lg:gap-16 mb-16">
        ...
        <div className="flex-1 w-full">
          <div className="relative rounded-xl overflow-hidden bg-gray-100 shadow-inner group">
            <img src={postDetail.img} alt={postDetail.title} className="w-full h-full object-cover aspect-[4/3] md:aspect-[3/2]" />
        ...
        <div className="w-full lg:w-[400px] flex flex-col justify-between py-2">
          <div>
            ...
            <div className="flex items-center gap-3 mb-8">
              <div className="w-12 h-12 bg-emerald-50 rounded-full flex items-center justify-center text-xl">😊</div>
              <div>
                <h4 className="font-bold text-gray-800">{postDetail.author}</h4>
                <p className="text-xs text-gray-400">작성일 {postDetail.date}</p>
              </div>
            </div>

            {/* ✅ 찜 버튼: 상태에 따라 하트 색상 변경 */}
            <button 
              onClick={() => setIsLiked(!isLiked)}
              className={`w-12 h-12 border rounded-full flex items-center justify-center transition-all active:scale-90 mb-10 ${
                isLiked ? "border-red-500 text-red-500 bg-red-50" : "border-gray-200 text-gray-300 hover:text-red-500"
              }`}
            >
          ...
          <div className="mt-auto">
            <h3 className="text-xl font-bold mb-4 text-gray-800 underline decoration-emerald-400 underline-offset-8">✏️ {postDetail.title}</h3>
            <div className="flex flex-wrap gap-2">
              {postDetail.tags.map(tag => (
                <span key={tag} className="text-xs md:text-sm font-semibold text-emerald-600 bg-emerald-50 px-3 py-1 rounded-md">#{tag}</span>
              ))}
            </div>
          </div>
        </div>
      ...
      <div className="border-t border-gray-100 pt-10">
        <h4 className="font-bold text-lg mb-6">댓글 <span className="text-emerald-500">{comments.length}</span></h4>
        
        {/* 댓글 입력 */}
        <div className="border border-gray-200 rounded-xl p-4 mb-10 focus-within:ring-2 focus-within:ring-emerald-100 focus-within:border-emerald-500 transition-all">
          <textarea 
            value={newComment}
          ...
          <div className="flex justify-end mt-2">
            <button onClick={handleCommentSubmit} className="bg-emerald-600 text-white px-6 py-2 rounded-lg text-sm font-bold hover:bg-emerald-700 transition-colors">등록</button>
          </div>
        </div>
        ...
        <div className="space-y-4">
          {comments.map((comment) => (
            <div key={comment.id} className="border border-gray-100 rounded-xl p-4 md:p-6 bg-gray-50/50 hover:bg-gray-50 transition-colors">
              <div className="flex justify-between items-start mb-3">
                <div className="flex items-center gap-3">
                  <div className="w-8 h-8 bg-white rounded-full flex items-center justify-center text-xs border border-gray-100">👤</div>
                  <div>
                ...
                <div className="flex gap-3 text-xs font-semibold">
                  {editingId === comment.id ? (
                    <>
                      <button onClick={() => handleSaveEdit(comment.id)} className="text-blue-600">저장</button>
                      <button onClick={() => setEditingId(null)} className="text-gray-400">취소</button>
                    </>
                  ) : (
                    <>
              ...
              {editingId === comment.id ? (
                <textarea 
                  ...
                  className="w-full p-4 border border-gray-200 rounded-lg text-sm outline-none bg-white focus:ring-2 focus:ring-blue-100 focus:border-blue-400 transition-all"
                  rows="3"
                />
              ) : (
