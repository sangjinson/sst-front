...
const AIResultScheduleItem = ({
  ...
  return (
    <div
      ...
      onDrop={onDrop}
      onClick={onClick}
      className={`flex items-center gap-3 px-3 py-5 rounded-xl border transition cursor-pointer ${
        isSelected
    ...
    >
      {/* 순서 번호 */}
      <div className="w-7 h-7 rounded-full bg-[#0F9B73] text-white text-xs flex items-center justify-center shrink-0 font-bold">
        {idx + 1}
      </div>
      ...
      <div className="flex-1 min-w-0">
        <div className="flex items-center gap-1 mb-1">
          <span className={`text-[10px] px-1.5 py-0.2 rounded-full ${
            CAT_KOR_COLOR_MAP[item.category] ?? CAT_COLOR_MAP[item.category] ?? TYPE_COLOR[item.type] ?? 'bg-green-100 text-[#0F9B73]'
          }`}>
