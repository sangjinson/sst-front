...
export const useAuth = () => {
  ...
  const logoutMutation = useMutation({
    ...
    onSuccess: (currentPath) => {
      ...
      const restrictedPaths = ['/plan', '/showcase/hotplace/write', '/showcase/life/write', '/admin'];
      
      if (restrictedPaths.some(path => currentPath.startsWith(path))) {
        // 관리자 페이지 등 제한된 경로에서 로그아웃하면 즉시 랜딩 페이지(/)로 이동!
        window.location.href = '/';
      } else {
        // 일반 접근 가능 페이지(예: 메인화면, 커뮤니티 등)에서는 현재 페이지 새로고침
