...
const generateReviews = (foodName) => [
  { user: "미식가123", rating: 5, comment: `${foodName} 진짜 맛있어요! 강력 추천합니다.` },
  { user: "여행중인밥", rating: 4, comment: "분위기도 좋고 음식도 맛있었어요. 또 오고 싶네요." },
  { user: "동네주민", rating: 5, comment: "여기 단골입니다. 항상 맛이 일정하고 서비스도 좋아요." },
];
...
export default function FoodView() {
  ...
  const id = searchParams.get("id"); // ✅ URL에서 ?id= 읽기
  ...
  useEffect(() => {
    ...
    const allFoods = getFoodDataByRegion(currentRegion);
      ...
      <div className="min-h-screen bg-[#f8f6f0]">
        <div className="food-view-wrap">
          ...
          <p className="text-sm text-gray-400 mb-4">
            ...
            {" > "}
            <span className="cursor-pointer hover:text-[#c8860a] transition-colors" onClick={() => navigate(`/${currentRegion}`)}>{currentRegion}</span>
            {" > "}
          ...
          <div className="relative rounded-2xl overflow-hidden mb-6 h-64 md:h-96">
            ...
            <div className="absolute bottom-0 left-0 right-0 bg-gradient-to-t from-black/70 to-transparent p-5">
              ...
              <p className="text-white/80 text-sm mt-1">{currentRegion}</p>
            </div>
          ...
          {relatedItems.length > 0 && (
            <div className="bg-white rounded-2xl p-5 mb-6 shadow-sm">
              <h2 className="text-lg font-bold text-gray-900 mb-4">{currentRegion} 비슷한 음식점</h2>
